...
- 状态：已确认（2026-08-18，2026-08-21 更新为 DDD 分层，2026-09-12 对齐现状：分层域扩至 10 个，2026-09-18 登记 industry 域：扩至 11 个，2026-09-22 登记 wiki 域：扩至 12 个）
...
- **DDD 分层域**（`user`、`conversation`、`market`、`valuation`、`portfolio`、`journal`、`allocation`、`screening`、`skill`、`mcp`、`industry`、`wiki`，共 12 个）：涉及持久化/多源编排的信息处理域，
...
### web（接入层）
- **作用**：HTTP 边界。`@RestController` 共 14 个（Auth/UserAdmin/Conversation/Market/Health/Valuation/Portfolio/Journal/Allocation/Screening/McpConfig/SkillConfig/Industry/Wiki）、`@RestControllerAdvice`（`GlobalExceptionHandler` 异常→HTTP 状态映射）、`@Component`（`InvestAguiRuntimeContextResolver`：AG-UI 运行时上下文解析）、Web 专属响应体（`ApiError`）与出入参 DTO（`web.dto`）。
