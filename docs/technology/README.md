...
├── modules/                 ← 二、技术视角的模块划分与设计（01–08 通用机制篇 + 09–14 业务域篇）
...
后端按「DDD 洋葱分层 + 独立能力域」划分（ArchUnit 强制）；前端按页面 + 同源反代组织。模块篇目分**通用机制篇（01–08，跨业务域的横切机制）**与**业务域篇（09–14，DDD 各能力域）**，各技术模块的设计文档：
