...
def run_backfill(runner, task, start: str, end: str):
    ...
    if unsupported:
        raise ValueError(
            f"任务 {task.task_code} 的源 {', '.join(unsupported)} 不支持区间回填"
            "（supports_range=False），拒绝 backfill"
        )
    return runner.run(task, mode=MODE_BACKFILL,
                      params={"start": normalize_date(start, "start"),
