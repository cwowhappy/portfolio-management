...
export function ChartToolRenderers() {
  useRenderTool({
    ...
    render: (p) => <ChartCard status={p.status} result={p.result} name={p.name} parameters={p.parameters} builder={buildCandlestickOption as ChartCardBuilder} />,
  });
  useRenderTool({
    name: "get_valuation",
    parameters: ValuationParamsSchema,
    render: (p) => <ChartCard status={p.status} result={p.result} name={p.name} parameters={p.parameters} builder={buildLineOption as ChartCardBuilder} />,
  });
  useRenderTool({
    name: "get_market_overview",
    parameters: OverviewParamsSchema,
    render: (p) => <ChartCard status={p.status} result={p.result} name={p.name} parameters={p.parameters} builder={buildBarOption as ChartCardBuilder} />,
  });
  return null;
}
