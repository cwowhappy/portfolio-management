...
export default function RulePanel({ rules, onChanged }: {
  ...
  // 表单保存与列表启停/删除各持一个防线：错误就近展示（表单卡内 / 列表区顶部）
  const form = useSaveAction();
  const rows = useSaveAction("操作失败");
  const { setError } = form;
  ...
  const toggle = (r: PrincipleRuleView) => {
    void rows.run(async () => {
  ...
  const remove = (r: PrincipleRuleView) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2" data-testid="wiki-rule-list">
        {rows.error && <div className="text-sm text-[color:var(--color-down)]">{rows.error}</div>}
