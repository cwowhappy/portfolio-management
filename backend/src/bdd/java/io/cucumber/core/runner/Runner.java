...
/**
 * 【Cucumber 7.34.7 缺陷遮蔽补丁 — 仅此一处与上游不同】
 *
 * <p>上游 {@code Runner#localeForPickle} 用 {@code new Locale.Builder().setLanguage(language)}
 * 构造 Locale。gherkin 的中文方言键是 "zh-CN"（含区域子标签），{@code setLanguage} 只接受
 * 纯语言子标签，直接抛 {@code IllformedLocaleException: Ill-formed language: zh-CN}，
 * 导致任何 {@code # language: zh-CN}（以及 zh-TW / en-Scouse 等含连字符方言）的 feature
 * 在场景执行前即失败。上游 main 分支仍未修复。
 *
 * <p>本类与 cucumber-core 7.34.7 的 Runner 逐字一致，仅把 localeForPickle 改为
 * {@code Locale.forLanguageTag(language)}（支持 BCP-47 完整标签，对 "en" 等纯语言码行为不变）。
 * 放在 bdd 测试源集：Gradle 测试类路径中测试类目录优先于依赖 jar，运行时以本类为准；
 * 生产与 integrationTest 类路径完全不受影响。Cucumber 升级修复后应删除本类。
 */
public final class Runner {
    ...
    private Locale localeForPickle(Pickle pickle) {
        ...
        // 补丁点：上游为 new Locale.Builder().setLanguage(language).build()，
        // 对 "zh-CN" 等含区域的方言键抛 IllformedLocaleException；forLanguageTag 支持完整 BCP-47 标签
